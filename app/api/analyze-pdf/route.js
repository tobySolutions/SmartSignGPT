...
Analyze the contract and fill each section with relevant findings. Keep each finding clear and concise. For the enforceability section, provide a well-structured paragraph that addresses all the points mentioned. Return the response as a valid JSON object.`
    ...
    const structuredAnalysis = {
      ...
      importantClauses: analysis.importantClauses.findings.map(f => `- ${f}`).join('\n'),
      potentialRisks: analysis.potentialRisks.findings.map(f => `- ${f}`).join('\n'),
      paymentTerms: analysis.paymentTerms.findings.map(f => `- ${f}`).join('\n'),
      terminationConditions: analysis.terminationConditions.findings.map(f => `- ${f}`).join('\n'),
